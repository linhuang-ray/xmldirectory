...
class Ion_auth_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        $company_data = array(
            ...
            'xml_key' => $this->hash_password($additional_data['company'], $this->salt())
        );
        $company_duplicate_check = $this->db->select('id')
                ->where(array('name' => $additional_data['company']))
                ->limit(1)
                ->get($this->tables['company']);
        if ($company_duplicate_check->num_rows() > 0) {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $this->trigger_events('extra_where');
        //2013 08 14
        //add first name and last name columns in query
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, company, first_name, last_name')
    ...
    public function users($groups = NULL) {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            $this->db->join(
                    $this->tables['company'], $this->tables['users'] . '.company = ' . $this->tables['company'] . '.id', 'inner'
            );
        }
    ...
    public function update($id, array $data) {
        ...
        //first check if the new name exists
        if (isset($data['company'])) {
            $c_x = $this->db->select('id', 'name')
            ...
            $company = $this->db->select('id', 'name')
                    ->where(array('id' => $user->company))
                    ->limit(1)
                    ->get('company');
            $c_r = $company->row();
            $this->db->update('company', array('name' => $data['company']), array('id' => $c_r->id));
            $data['company'] = $c_r->id;
        }
    ...
    public function delete_user($id) {
        ...
        //remove user company from company table
        $this->remove_from_company($id);
    ...
    /*
     * remove user company when delete the user
     * 
     */

    public function remove_from_company($id) {
        ...
        if ($company_id->num_rows() > 0) {
            $c_id = $company_id->row()->company;
            $this->db->where('id', $c_id)
                    ->delete($this->tables['company']);
            return true;
        } else {
            return false;
        }
